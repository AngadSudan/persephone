...
const invalidateInterviewerCaches = async (
  interviewerId: string,
): Promise<void> => {
  await invalidateManyCacheKeysSafe([
    getInterviewerProfileCacheKey(interviewerId),
  ]);
};
...
class InterviewerController {
    ...
    try {
      ...
      const updatedInterviewer = await prismaClient.interviewer.update({
        ...
        data: {
          ...
          userInfo:
            (data.userInfo?.trim() || undefined) ?? dbInterviewer.userInfo,
          headline:
            (data.headline?.trim() || undefined) ?? dbInterviewer.headline,
        },
      });
  ...
  async getFullProfile(req: Request, res: Response) {
    try {
      ...
      console.log("Inside INterviewer controller");
      const interviewerProfileCacheKey =
        getInterviewerProfileCacheKey(interviewerId);
      ...
      if (cachedProfile !== null) {
        return res
          .status(200)
          .json(
            apiResponse(200, "Interviewer data found (Cache)", cachedProfile),
          );
      }
