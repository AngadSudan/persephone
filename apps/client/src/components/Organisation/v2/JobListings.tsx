...
export default function JobListings() {
    ...
    useEffect(() => {
        ...
        async function fetchJobListingsCount() {
            try {
                const res = await axiosInstance.get("/api/v1/jobListing/get-job-listings-count");
                if (mounted) {
                    setTotalListings(res.data?.data?.totalListings ?? 0);
                }
            } catch (error) {
                console.error("Failed to fetch job listings count", error);
                if (mounted) {
                    setTotalListings(0);
                }
            }
