...
async function handleCreateInterviewer(data: {
  name: string;
  username: string;
  email: string;
}) {
  return axiosInstance.post(`/api/v1/organizations/create-interviewer`, data, {
...
export default function InterviewersTable() {
  ...
  async function fetchInterviewers() {
    try {
      const res = await axiosInstance.get(
        `/api/v1/organizations/interviewers`,
        { withCredentials: true },
      );
      const data = res.data;
  ...
  return (
    <>
      <div className="space-y-5 mono">
        <div className="flex items-center justify-between gap-2 mono">
          <button
            className={`${Colors.background.primary} ${Colors.text.primary} ${Colors.properties.interactiveButton} p-2 rounded-md`}
            onClick={() => {
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <p
                  ...
                  className={`font-mono ${Colors.text.primary}`}
                >
                  {selectedInterviewer.name}
                </p>
                <p
                  style={{ fontSize: "0.75rem" }}
                  className={`${Colors.text.special} opacity-80 font-mono`}
                >
