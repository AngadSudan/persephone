...
export default function WishlistUsersTable({
  ...
  return (
    <div
      className={`overflow-hidden rounded-2xl ${Colors.background.primary} ${Colors.border.specialThin}`}
    >
      <div
        className={`grid grid-cols-12 px-4 py-3 text-xs uppercase ${Colors.text.secondary} ${Colors.border.defaultThinBottom}`}
      >
      ...
      <div>
        {entries.map((entry, idx) => (
          ...
          >
            ...
            <span
              className={`col-span-4 truncate text-sm ${Colors.text.secondary}`}
            >
